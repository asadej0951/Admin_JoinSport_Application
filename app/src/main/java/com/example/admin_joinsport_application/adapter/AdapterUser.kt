...
class AdapterUser (var ct :Context,private var Data:ArrayList<ResponseUser>):RecyclerView.Adapter<ViewHolderUser>(){
    ...
    override fun onBindViewHolder(holder: ViewHolderUser, position: Int) {
        holder.name.text = "คุณ "+Data[position].u_name+" "+Data[position].u_lname
        ...
        holder.itemView.setOnLongClickListener {
            ...
            builderSingle.setItems(animals){
                ...
                when(which){
                    0->{
                        DialogMessage(Data.get(position).u_id.toInt())
                    }
                }
            }
            builderSingle.show()
    ...
    private fun setAPIDelete(id: Int) {
        mPresenterPost.deleteUserPresenterRX(id,this::deleteNext,this::deleteError)

    }

    private fun deleteNext(responseUser: ResponseUser) {
...
class ViewHolderUser(item: View):RecyclerView.ViewHolder(item){
    var image = item.findViewById<ImageView>(R.id.img_user)
    var name = item.findViewById<TextView>(R.id.name_user)
}
